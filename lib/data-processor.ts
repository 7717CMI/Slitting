    ...
    if (effectiveAggregationLevel !== undefined && effectiveAggregationLevel !== null) {
      ...
      if (record.is_aggregated === true) {
        ...
        if (selectedLevel1Segments.length > 0) {
          ...
          // Support mixed selections - some geography names and some country names
          if (isRegionalSegmentType) {
            const regionalGeographies = ['North America', 'Europe', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa', 'Global']

            // Check if this record matches ANY of the selected segments
            // Each selection can be either a geography name or a segment/country name
            let matchesAnySelection = false

            for (const selectedSeg of selectedLevel1Segments) {
              // Check if this selection is a geography name
              if (regionalGeographies.includes(selectedSeg)) {
                // Match by geography
                if (record.geography === selectedSeg) {
                  matchesAnySelection = true
                  break
                }
              } else {
                // Selection is a segment/country name - match by segment
                if (record.segment === selectedSeg) {
                  matchesAnySelection = true
                  break
                }
                // Also check segment hierarchy for partial matches
                const hierarchy = record.segment_hierarchy
                if (hierarchy.level_1 === selectedSeg || hierarchy.level_2 === selectedSeg) {
                  matchesAnySelection = true
                  break
                }
              }
  ...
  return years.map(year => {
    ...
    records.forEach(record => {
      ...
      records.forEach(record => {
        ...
        if (isLevel1) {
        ...
        } else if (isRegionalSegmentType && hasUserSelectedSegments) {
          // SPECIAL CASE: For regional segment types with explicit selections,
          // each selected segment becomes its own bar
          const regionalGeographies = ['North America', 'Europe', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa', 'Global']

          // Find which selected segment this record matches
          let matchedSelection: string | null = null

          for (const selectedSeg of selectedSegmentNames) {
            // Check if selection is a geography name
            if (regionalGeographies.includes(selectedSeg)) {
              // Match by geography
              if (record.geography === selectedSeg) {
                matchedSelection = selectedSeg
                break
              }
            } else {
              // Selection is a segment/country name - match by segment
          ...
          if (matchedSelection) {
            key = matchedSelection
          } else {
            return // Skip - no match found
          }
...
export function prepareIntelligentMultiLevelData(
  ...
  records.forEach(record => {
    ...
    // 3. Mixed - some geography names and some country names
    if (isRegionalSegmentType && hasExplicitLevel1Selection) {
      const regionalGeographies = ['North America', 'Europe', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa', 'Global']

      // For regional segment types, each selection should become its own bar
      // Check if this record matches ANY of the selected segments
      let matchedSelection: string | null = null
